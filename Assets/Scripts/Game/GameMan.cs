...
public class GameMan : ManagerBehaviour {
    ...
    private QUALITY DetermineRandomQuality(LEVEL level, List<QUALITY> QualitiesToConsider) {
        ...
        foreach (QUALITY _quality in QualitiesToConsider) {
            maxQualityNumber += (int) (DROP_CHANCE[(int) level, (int) _quality] * 100);
        }
        ...
        foreach (QUALITY _quality in QualitiesToConsider) {
            if ((qualityNumber -= (int) (100 * DROP_CHANCE[(int) level, (int) _quality])) < 0) {
                quality = _quality;
                break;
            }
        }
    ...
    private Unit HandleSpawnRandomUnitEvent() {
        ...
        return SpawnRandomUnit(DetermineRandomQuality(BoardMan.Instance.currentLevel, allQualities));
    }
